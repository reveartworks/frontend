...
import PurchaseDataTable from "../../Component/PurchaseDataTable";

export function PurchaseEnquiries(props) {
  var imageList = [];
  ...
  const getPurchaseEnquiries = async (e) => {
    ...
    if (passCode) {
      try {
        setLoading(true);
        const result = await apiRequest("POST", "/getPurchaseEnquiries", {
          passCode: passCode,
        });
        console.log(result);
        ...
        >
          ...
          >
            ...
            {loading ? (
              ...
              >
                <PurchaseDataTable rows={rows}></PurchaseDataTable>
              </div>
            ) : null}
          </div>
        </div>
